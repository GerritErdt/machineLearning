...
class MagicStereoDataset(data.Dataset):
    def __init__(self, m1, m2, y):
        ...
        self.background_intensity = torch.mode(self.images_m1.view(-1))[0].item() if self.images_m1.numel() > 0 else 0.0
    ...
    def __getitem__(self, idx):
        ...
        return tg_data.Data(x=x_sparse, edge_index=edge_index, pos=pos_sparse, y=y)
...
def preprocess_images(p_m1, p_m2, g_m1, g_m2, epsilon = 1e-8): 
    ...
    total_min = min(x_m1_train.min(), x_m2_train.min())
    total_max = max(x_m1_train.max(), x_m2_train.max())
    
    x_m1_train = (x_m1_train - total_min) / (total_max - total_min + epsilon)
    x_m1_test = (x_m1_test - total_min) / (total_max - total_min + epsilon)
    x_m2_train = (x_m2_train - total_min) / (total_max - total_min + epsilon)
    x_m2_test = (x_m2_test - total_min) / (total_max - total_min + epsilon)
...
def get_stereo_clean_dataset(num_samples = 10000, batch_size = 32):
    ...
    train_dataset = MagicStereoDataset(m1_train, m2_train, y_train)
    test_dataset = MagicStereoDataset(m1_test, m2_test, y_test)
