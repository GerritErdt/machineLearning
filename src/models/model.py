...
class GNNModel(nn.Module):
    def __init__(self, input_net_dropout, num_edge_convs, gnn_step_dropout, classifier_dropout, in_channels=2, internal_dimensions=64):
        ...
        # initially, the projection layers should zero the GNN output. This stabilizes the training in the beginning and improves backward propagation
    ...
    def _init_weights(self, module): 
        if isinstance(module, nn.Linear):
            nn.init.kaiming_normal_(module.weight, mode='fan_in', nonlinearity='relu') # relu is close to gelu, thus a valid approximation and more suited that "blind" initialization
            
            if module.bias is not None:
                nn.init.zeros_(module.bias)
        
        # normalization should act as identity in the beginning
