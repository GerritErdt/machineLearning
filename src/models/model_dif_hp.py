...
class GNNModel(nn.Module):
    def __init__(self, input_net_dropout, num_edge_convs, gnn_step_dropout, classifier_dropout, activation_function, aggr_combination, in_channels=2, internal_dimensions=64):
        super().__init__()
        
        activation = activations[activation_function]
        self.aggr_list = aggr_combination.split("+")
        num_aggrs = len(self.aggr_list)
        multi_aggr = gnn.aggr.MultiAggregation(self.aggr_list)
        ...
        for _ in range(self.num_edge_convs):
            ...
            self.projs.append(nn.Sequential(  # only perform linear projection to reduce dimensionality (combine max/mean features)
                nn.Linear(num_aggrs * internal_dimensions, internal_dimensions),
            ))
        ...
        self.classifier = nn.Sequential(
            nn.Linear(2 * num_aggrs * internal_dimensions, internal_dimensions),
            nn.LayerNorm(internal_dimensions),
            activation,
        ...
        print(f"Initialized GNNModel with activation_function={activation_function}, aggr_combination={aggr_combination}. Trainable parameters: {sum(p.numel() for p in self.parameters() if p.requires_grad)}", flush=True)
    ...
    def forward(self, x, edge_index, batch, num_graphs):
        ...
        pool_dict = {
            ...
            "add": gnn.global_add_pool
        }

        # jumping knowledge of initial features against over-smoothing
        x_input_pooled = [pool_dict[aggr](x, batch, size=num_graphs) for aggr in self.aggr_list]
        ...
        # Dynamisches Feature Pooling der GNN Features
        x_gnn_pooled = [pool_dict[aggr](x, batch, size=num_graphs) for aggr in self.aggr_list]

        # Konkateniere alle gepoolten Listen-Elemente (Input + GNN) auf der Feature-Dimension
        x_pooled = torch.cat(x_input_pooled + x_gnn_pooled, dim=1)
...
def learn(model, train_loader, val_loader, test_loader, epochs, lr_max, l2_reg, pos_weight, early_stopping_patience=5, trial=None):
    ...
    grace_period = int(epochs * 0.5)  # the LR scheduler takes some time to ramp up
...
def objective(trial, train_loader, val_loader, test_loader, epochs, pos_weight):
    config = {
        ...
        "activation_function": trial.suggest_categorical("activation_function", ["GELU", "ReLU", "LeakyReLU", "SELU", "RReLU"]),
        "aggr_combination": trial.suggest_categorical("aggr_combination", ["max+mean", "max+add", "mean+add", "max+mean+add"]),
    ...
    model = GNNModel(
        ...
        aggr_combination=config["aggr_combination"]
    )
